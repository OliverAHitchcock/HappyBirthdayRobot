...
def _resize_image_array(img_bhwc: np.ndarray, target_size: tuple[int, int]) -> np.ndarray:
    """Pad a batch of images to target size by adding black bars on the shorter side.

    Args:
        img_bhwc: (B, H, W, C) uint8 numpy array
        target_size: (height, width) target size

    Returns:
        (B, target_height, target_width, C) uint8 numpy array with black padding on shorter side
    """
    ...
    # Create padded batch with zeros (black padding)
    padded_batch = np.zeros((b, target_h, target_w, c), dtype=np.uint8)

    # Calculate padding to center the image (add black bars on shorter side)
    pad_top = (target_h - h) // 2
    pad_left = (target_w - w) // 2

    # Place original image in center
    padded_batch[:, pad_top:pad_top + h, pad_left:pad_left + w, :] = img_bhwc
...
class GrootPackInputsStep(ProcessorStep):
    ...
    # Image size for resizing (to ensure all cameras have same resolution)
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        if img_keys:
            ...
            # Find maximum dimensions across all cameras
            max_h = max(cam.shape[1] for cam in cams)  # max height
            max_w = max(cam.shape[2] for cam in cams)  # max width

            # Pad all cameras to the maximum dimensions
            cams = [_resize_image_array(cam, (max_h, max_w)) for cam in cams]
        ...
        for v in list(obs.values()) + [transition.get(TransitionKey.ACTION)]:
            if isinstance(v, torch.Tensor):
                ...
                device = v.device
                break
        if bsz is None and "video" in obs:
            if isinstance(obs["video"], np.ndarray):
                bsz = obs["video"].shape[0]
            elif isinstance(obs["video"], list):
                bsz = len(obs["video"])
        if bsz is None:
            bsz = 1
