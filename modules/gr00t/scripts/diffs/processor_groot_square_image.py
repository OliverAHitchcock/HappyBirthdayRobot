...
def _resize_image_array(img_bhwc: np.ndarray, target_size: tuple[int, int]) -> np.ndarray:
    """Resize a batch of images to target size using PIL.
    ...
    Returns:
        (B, target_height, target_width, C) uint8 numpy array
    """
    ...
    resized_batch = np.zeros((b, target_h, target_w, c), dtype=np.uint8)
    for i in range(b):
        img_pil = Image.fromarray(img_bhwc[i])
        # Use LANCZOS for high-quality downsampling, BILINEAR for upsampling
        resample_method = Image.LANCZOS if (h > target_h or w > target_w) else Image.BILINEAR
        img_resized = img_pil.resize((target_w, target_h), resample=resample_method)
        resized_batch[i] = np.array(img_resized)
...
class GrootPackInputsStep(ProcessorStep):
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        if img_keys:
            ...
            # Resize all cameras to the same resolution before stacking
            cams = [_resize_image_array(cam, self.image_size) for cam in cams]
